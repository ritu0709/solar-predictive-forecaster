...
if st.sidebar.button("Run Predictive Asset Audit"):
    with st.spinner("Executing multi-model time-series reduction rules..."):
        ...
        st.pyplot(fig)
# ... (Keep everything above this line in app.py exactly the same) ...
        
        # Render plot in Streamlit
        st.pyplot(fig)
        
        # NEW SECTION: Display week-by-week data breakdown for the client
        st.markdown("### 📊 Detailed Week-by-Week Forecast Matrix")
        st.markdown("Below are the exact predicted values generated by the model for the upcoming year.")
        ...
        # Format metrics for high-end corporate presentation
        forecast_df["Predicted Energy Yield"] = forecast_df["Predicted Energy Yield"].map("{:,.2f} kWh".format)
        
        # Display as an interactive, sortable data table inside your app
